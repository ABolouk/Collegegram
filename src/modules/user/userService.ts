...
export class UserService {
  ...
  async login(loginDto: LoginDtoType) {
    const user = await (isUserEmail(loginDto.authenticator)
      ? this.userRepository.findByEmail(loginDto.authenticator)
      : this.userRepository.findByUsername(loginDto.authenticator));
    if (!user) {
  ...
  async signup(dto: signupDto) {
    ...
    const hashedPassword = await Password.Hashed.mk(dto.password);
  ...
  async forgetPassword({ authenticator }: ForgetPasswordDto) {
    ...
    const oneTimeLink = createOneTimeLink(
      `${process.env.HOST_NAME}/user/${resetPasswordRoute}`,
      user,
      expiresIn
    );
  ...
  async resetPassword(
    ...
    this.userRepository.updatePasswordById(
      user.id,

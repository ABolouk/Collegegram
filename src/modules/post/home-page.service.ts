...
export class HomePageService {
    ...
    async getHome(dto: homePageDtoType) {
        ...
        const homePagePosts = await Promise.all(posts.map(async (x) => ({
            id: x.id,
            familyName: await this.userLowService.getFamilyNameById(x.userId),
            userName: (await this.userLowService.getUserById(x.userId)).username,
            tags: x.tags,
            photos: x.photos
        })))
        return {

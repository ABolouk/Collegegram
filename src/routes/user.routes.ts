...
import {closeFriendDto} from "../modules/user/closeFriend/dto/close-friend.dto";
...
export const makeUserRouter = (userHighService: UserHighService, sessionLowService: SessionLowService, userLowService: UserLowService, jwtService: JwtService, followHighService: FollowHighService, blockHighService: BlockHighService, closeFriendHighService : CloseFriendHighService) => {
    const app = Router();
    ...
    app.post("/addCloseFriend", loginMiddle(userLowService, sessionLowService), (req, res) => {
        const userId = req.user.id;
        const userName = req.body.userName;
        const dto = closeFriendDto.parse({userId, userName})
        handleExpresss(res, () => closeFriendHighService.addCloseFriend(dto))
    })
